...
const EMPTY = { nom: '', description: '', prix_achat: '', prix_vente: '', sku: '', categorie_id: null, actif: true }

export default function ProductsPage() {
  const apiBase = (import.meta.env.VITE_API_URL || 'http://localhost:8000/api').replace(/\/+$/, '')
  // remove trailing /api or /api/v1 (with optional trailing slash)
  const apiHost = apiBase.replace(/\/api(\/v\d+)?$/i, '')

  function getImageUrl(img) {
    if (!img) return null
    if (typeof img !== 'string') return null
    if (/^https?:\/\//.test(img)) return img
    if (img.startsWith('/storage/')) return `${apiHost}${img}`
    if (img.startsWith('storage/')) return `${apiHost}/${img}`
    if (img.includes('/storage/')) return `${apiHost}/${img.replace(/^\//, '')}`
    // typical stored path like 'products/xxx.jpg'
    return `${apiHost}/storage/${img}`
  }
  ...
  async function load() {
    setLoading(true)
    try {
      const { data } = await productsApi.list({ inactifs: true })
      const rowsData = Array.isArray(data) ? data : data.data || []
      setRows(rowsData)
      try {
        console.debug('Products images sample:', rowsData.slice(0,10).map(r => ({ id: r.id, image: r.image, image_url: r.image_url, imageUrl: getImageUrl(r.image_url || r.image) })))
      } catch (e) { console.debug('Products sample log error', e) }
  ...
  function openEdit(row) {
    setForm({
      ...
      sku: row.sku || '', categorie_id: row.categorie_id || null, actif: !!row.actif
    })
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      if (form.image instanceof File) {
        ...
        if (modal.data) await productsApi.update(modal.data.id, fd)
        else await productsApi.create(fd)
      } else {
        if (modal.data) await productsApi.update(modal.data.id, payload)
  ...
  async function handleDelete() {
    setDeleting(true)
    try {
      await productsApi.destroy(confirm.id)
